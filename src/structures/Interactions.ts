...
export class Interactions {
    ...
    public async createGlobalCommand<T>(command: ICommand): Promise<T> {
        return (await Wrap(
            this.request.POST<T>({
    ...
    public async getGlobalCommand<T>(commandId: Snowflake): Promise<T> {
        return (await Wrap(
            this.request.GET<T>({
    ...
    public async editGlobalCommand<T>(commandId: Snowflake, command: ICommand): Promise<T> {
        return (await Wrap(
    ...
    public async deleteGlobalCommand(commandId: Snowflake): Promise<void> {
        await this.request.DELETE<ApplicationCommand>({
            route: this.routes.deleteGlobalApplicationCommand(this.options.botID, commandId),
            contentType: APPLICATION_TYPE.JSON,
        });
    }
    ...
    public async createGuildCommand<T>(guildId: Snowflake, command: ICommand): Promise<T> {
        return (await Wrap(
            this.request.POST<T>({
    ...
    public async getGuildCommands<T>(guildId: Snowflake): Promise<T> {
        return (await Wrap(
            this.request.GET<T>({
    ...
    public async deleteGuildCommand(guildId: Snowflake, commandId: Snowflake): Promise<void> {
        await this.request.DELETE<ApplicationCommand>({
    ...
    public async overwriteGuildCommands(
        ...
        await this.request.PUT<ApplicationCommand>({
            route: this.routes.bulkOverwriteGuildApplicationCommands(this.options.botID, guildId),
            contentType: APPLICATION_TYPE.JSON,
            data: commands,
        });
    }
    ...
    public async getGuildCommand<T>(guildId: Snowflake, commandId: Snowflake): Promise<T> {
        return (await Wrap(
            this.request.GET<T>({
                publicKey: true,
    ...
    public async getGuildCommandPermissions<T>(guildId: Snowflake): Promise<T> {
        return (await Wrap(
            this.request.GET<T>({
                publicKey: true,
